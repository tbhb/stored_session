...
class StoredSession::StoreTest < ActionDispatch::IntegrationTest
  ...
  test "prevents session fixation" do
    with_test_route_set do
      ...
      assert_nil StoredSession::Session.find_by(sid: sid.private_id)
      ...
      assert_equal(
        { "foo" => "bar" },
        StoredSession::Session.read(Rack::Session::SessionId.new(cookies["_session_id"]).private_id)
      )
    end
  end
  ...
  test "sessions are indexed by hashed session" do
    with_test_route_set do
      ...
      session = StoredSession::Session.last
